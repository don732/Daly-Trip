...
import { findTripByCode, getTrip, loadState, saveState, saveTrip, STORAGE_KEY } from '@/localStore'
...
describe('localStore', () => {
  ...
  it('saveTrip and getTrip round-trip through state', () => {
    const trip = createTestTrip()
    const base = { trips: {}, activeTripId: null, merit: [] }
  ...
  it('persists state to localStorage', () => {
    const trip = createTestTrip()
    saveTrip({ trips: {}, activeTripId: null, merit: [] }, trip)
  ...
  it('saveState writes merit array', () => {
    const state = { trips: {}, activeTripId: null, merit: [{ nick: 'A', trips: 1, points: 10 }] }
    saveState(state)
